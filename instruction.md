...
Он описывает, какие шаги нужно выполнить и какой результат должен быть у каждого шага.
...
**Вариант составления тест-кейсов**
![Тест-кейсы](Test_cases.png)

## Тестирование и заведение баг-репортов

Для проведеняи тестирования и заведения баг-репортов рекомендуется использовать [Trello](https://trello.com). Инструмент максимально удобен для заполнения отчета о дефектах.

**Отчет о дефекте** — это документ, который описывает шаги воспроизведения дефекта,фактический и ожидаемый результат, серьезность дефекта и приоритет устранения.

_**Атрибуты отчета о дефектах**_ 
* **Заголовок** - Пропадают заголовки на главной странице при скролле
* **Предусловие** - Открыть главную страницу и отключить интернет
* **Шаги воспроизведения** - Проскроллить главную страницy
* **Фактический результат** - Пропадают заголовки на главной странице при скролле
* **Ожидаемый результат** - Заголовки не пропадают на главной странице при скролле
* **Окружение**- macOS Big Sur, Chrome 99.0.4844.84

При создании баг-репорта не стоит забывать и о его приоритетности (как быстро надо починить) и серьезности (насколько баг мешает пользователю).
